...
MODEL = "qwen3.5-plus"
# 先验证前几页不会误检，再验证前 40 页里最早的带图页。
TEST_PAGES = [0, 1, 2, 3, 4, 15, 26]
...
def detect_figures_json(client, page_png_bytes, page_num):
    ...
    detect_bytes, detect_mime, detect_size = prepare_image_for_model(page_png_bytes, max_side=1600)
    prompt = (
        "检测这个扫描书页中所有非文字的视觉元素（插图、照片、图表、图形、地图、波形图等）。\n"
        "不要包括纯文字区域、页眉页脚、页码、条形码。\n"
        "只框住图片主体本身，不要把图题、图注、相邻正文、页码或边距包含进框里。\n"
        "返回JSON数组，每个元素格式：\n"
        '{"bbox": [x1, y1, x2, y2], "desc": "简短中文描述"}\n'
        "其中 bbox 使用归一化坐标，左上角为(0,0)，右下角为(1000,1000)。\n"
        "如果页面上没有图片/图表，返回空数组：[]\n"
        "只输出JSON，不要其他文字。"
    )
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "user", "content": [
                ...
                {"type": "text", "text": f"输入图尺寸：{detect_size[0]}x{detect_size[1]}\n{prompt}"},
            ]},
        ],
        temperature=0.1,
        max_tokens=1024,
...
def detect_figures_qwenvl_md(client, page_png_bytes, page_num):
    ...
    detect_bytes, detect_mime, _ = prepare_image_for_model(page_png_bytes, max_side=1600)
    response = client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role": "user", "content": [
...
def parse_bbox_json(raw_text):
    """解析 JSON 格式的 bbox 输出"""
    return parse_figure_detection_response(raw_text, (1131, 1600))
...
def crop_figure(page_png_bytes, bbox, img_width, img_height, is_normalized=False):
    ...
    if is_normalized:
        x1, y1, x2, y2 = bbox_to_pixels(bbox, img.width, img.height)
    else:
        x1, y1, x2, y2 = bbox_to_pixels([int(v) for v in bbox], img.width, img.height)
...
def main():
    ...
    for page_num in pages_to_test:
        ...
        detect_preview, _, detect_size = prepare_image_for_model(page_png, max_side=1600)
        ...
        print(f"🧪 检测输入尺寸: {detect_size[0]}x{detect_size[1]}, {len(detect_preview) / 1024:.1f} KB")
        print(f"{'='*60}")
        ...
        try:
            ...
            print(f"  📝 原始输出:\n    {raw_a[:500]}")
            figures_a = parse_figure_detection_response(raw_a, detect_size)
            print(f"  📊 解析到 {len(figures_a)} 个区域")
            for i, fig in enumerate(figures_a):
                print(f"    [{i}] bbox={fig.get('bbox')}, desc={fig.get('desc', '')}")
                cropped = crop_figure(page_png, fig["bbox"], img_w, img_h, is_normalized=True)
                if cropped:
        ...
        try:
            ...
            print(f"  ⏱ {elapsed:.1f}s")
            print(f"  📝 原始输出:\n    {raw_c[:500]}")
            figures_c = parse_figure_detection_response(raw_c, detect_size)
        ...
        if page_num in [15, 26, 28, 33]:
            ...
            try:
                ...
                figures_b = parse_qwenvl_markdown_figures(raw_b, detect_size)
                print(f"  📊 解析到 {len(figures_b)} 个区域")
                for i, fig in enumerate(figures_b):
                    print(f"    [{i}] bbox={fig.get('bbox')}")
                    cropped = crop_figure(page_png, fig["bbox"], img_w, img_h, is_normalized=True)
                    if cropped:
                        save_path = OUTPUT_DIR / f"page{page_num+1}_B_fig{i+1}.png"
                        cropped.save(str(save_path))
                        print(f"    ✅ 已保存: {save_path.name} ({cropped.width}x{cropped.height})")
            except Exception as e:
                print(f"  ❌ 异常: {e}")
